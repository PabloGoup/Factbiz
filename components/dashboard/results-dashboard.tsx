...
export function ResultsDashboard() {
  ...
  useEffect(() => {
    ...
    if (snapshot.insights.source === "openai") {
      setInsightStatus("ready");
      setInsightMessage(`Insights generados con ${snapshot.insights.model ?? "OpenAI"}.`);
      return;
    }
    ...
    <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-6 xl:grid-cols-[1.2fr_0.8fr]">
        <Card>
          <div className="grid gap-8 lg:grid-cols-[320px_minmax(0,1fr)] lg:items-center">
            ...
            <div>
              ...
              />
              <div className="mt-4 inline-flex items-center gap-2 rounded-full border border-slate-200 bg-slate-50 px-4 py-2 text-xs font-semibold uppercase tracking-[0.18em] text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
                <Bot className="h-3.5 w-3.5" />
                {snapshot.insights.source === "openai" ? "Insights con OpenAI" : "Simulación local"}
              </div>
              {insightMessage ? (
                <p className="mt-3 text-xs text-slate-500 dark:text-slate-400">{insightMessage}</p>
              ) : null}
              <p className="mt-5 font-serif text-3xl font-semibold text-slate-950 dark:text-slate-50">
              ...
              <div className="mt-6 grid gap-3 md:grid-cols-3">
                ...
                <div className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
                  <p className="text-xs uppercase tracking-[0.16em] text-slate-500 dark:text-slate-400">Contexto</p>
                  <p className="mt-2 font-semibold text-slate-950 dark:text-slate-50">{snapshot.context.source}</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <Card>
          ...
          <p className="mt-4 text-sm leading-6 text-slate-600 dark:text-slate-300">{snapshot.context.narrative}</p>
          <div className="mt-6 grid gap-3 sm:grid-cols-2">
            {[
              ...
              <div key={label} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
                ...
                <p className="mt-2 text-xl font-semibold text-slate-950 dark:text-slate-50">{Number(value).toFixed(1)}/10</p>
              </div>
            ))}
          </div>
        </Card>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[0.95fr_1.05fr]">
        <Card>
          <p className="text-sm font-semibold uppercase tracking-[0.18em] text-slate-500 dark:text-slate-400">
            {snapshot.insights.source === "openai" ? "Hallazgos con IA" : "Hallazgos tipo IA"}
          </p>
            ...
            <div>
              ...
              <div className="mt-3 grid gap-3">
                {snapshot.insights.mainFindings.map((item) => (
                  <div key={item} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm leading-6 text-slate-600 dark:border-slate-800 dark:bg-slate-900 dark:text-slate-300">
                    {item}
                  </div>
                ))}
              </div>
            ...
            <div>
              ...
              <ul className="mt-3 space-y-3 text-sm leading-6 text-slate-600 dark:text-slate-300">
                {snapshot.insights.opportunities.map((item) => (
                  <li key={item} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 dark:border-slate-800 dark:bg-slate-900">
                    {item}
                  </li>
                ))}
              </ul>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-2">
        ...
        <Card>
          ...
          <div className="mt-5 space-y-4">
            {snapshot.scoreBreakdown.blocks.map((block) => (
              <div key={block.id} className="rounded-3xl border border-slate-200 bg-slate-50 p-5 dark:border-slate-800 dark:bg-slate-900">
